...
export class WebGLRenderer {
  ...
  private createShaders(): void {
    ...
    const instancedVertexShader = `#version 300 es
      ...
      void main() {
        ...
        vec2 worldPos = scaledPos + a_instanceTransform.xy;
        
        // Project to screen space
        vec4 clipPos = u_viewProjectionMatrix * vec4(worldPos, a_instanceDepth, 1.0);
        gl_Position = clipPos;
  ...
  private prepareInstanceData(elements: SpatialElement[]): Float32Array {
    // Each instance needs: transform(4) + color(4) + texCoords(4) + depth(1) = 13 floats
    const data = new Float32Array(elements.length * 13);
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      // Depth (for layering)
      data[offset + 12] = (element.layer ?? 0) * 0.001; // Convert layer to depth
    }
  ...
  private setupVertexAttributes(program: WebGLProgram): void {
    ...
    gl.vertexAttribPointer(program.attributes['a_instanceTransform'], 4, gl.FLOAT, false, 52, 0);
    ...
    gl.vertexAttribPointer(program.attributes['a_instanceColor'], 4, gl.FLOAT, false, 52, 16);
    ...
    gl.vertexAttribPointer(program.attributes['a_instanceTexCoords'], 4, gl.FLOAT, false, 52, 32);
    ...
    gl.vertexAttribPointer(program.attributes['a_instanceDepth'], 1, gl.FLOAT, false, 52, 48);
